...
namespace RunescapeServer.minigames.fightpits {
    public class FightPits {
        ...
        private void resetVariables(Player p) {
            ...
            p.setFrozen(false);
            p.removeTemporaryAttribute("unmovable");
            if (p.PlayerAttributes.AntiFirePotionCycle != null) { p.PlayerAttributes.AntiFirePotionCycle.stop(); }
            if (p.PlayerAttributes.SuperAntiPoisonPotionCycle != null) { p.PlayerAttributes.SuperAntiPoisonPotionCycle.stop(); }
